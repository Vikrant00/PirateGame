...
// JSON Utilities
TSharedPtr<FJsonObject> FPirateMCPCommonUtils::CreateErrorResponse(const FString& Message)
{
...
TSharedPtr<FJsonObject> FPirateMCPCommonUtils::CreateSuccessResponse(const TSharedPtr<FJsonObject>& Data)
{
    TSharedPtr<FJsonObject> ResponseObject = MakeShared<FJsonObject>();
...
void FPirateMCPCommonUtils::GetIntArrayFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName, TArray<int32>& OutArray)
{
    OutArray.Reset();
...
void FPirateMCPCommonUtils::GetFloatArrayFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName, TArray<float>& OutArray)
{
    OutArray.Reset();
...
FVector2D FPirateMCPCommonUtils::GetVector2DFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
FVector FPirateMCPCommonUtils::GetVectorFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
FRotator FPirateMCPCommonUtils::GetRotatorFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
// Blueprint Utilities
UBlueprint* FPirateMCPCommonUtils::FindBlueprint(const FString& BlueprintName)
{
    return FindBlueprintByName(BlueprintName);
}

UBlueprint* FPirateMCPCommonUtils::FindBlueprintByName(const FString& BlueprintName)
{
    ...
    if (!Blueprint)
    {
         UE_LOG(LogPirateMCP, Error, TEXT("FindBlueprintByName: Failed to find or load blueprint: %s"), *BlueprintName);
    }

    return Blueprint;
}

UEdGraph* FPirateMCPCommonUtils::FindOrCreateEventGraph(UBlueprint* Blueprint)
{
    if (!Blueprint)
    {
        return nullptr;
...
UK2Node_Event* FPirateMCPCommonUtils::CreateEventNode(UEdGraph* Graph, const FString& EventName, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
    ...
    for (UEdGraphNode* Node : Graph->Nodes)
    {
        ...
        {
            UE_LOG(LogPirateMCP, Display, TEXT("Using existing event node with name %s (ID: %s)"), 
    ...
    if (EventFunction)
    {
        ...
        UE_LOG(LogPirateMCP, Display, TEXT("Created new event node with name %s (ID: %s)"), 
            *EventName, *EventNode->NodeGuid.ToString());
    }
    else
    {
        UE_LOG(LogPirateMCP, Error, TEXT("Failed to find function for event name: %s"), *EventName);
    }
    
    return EventNode;
}

UK2Node_CallFunction* FPirateMCPCommonUtils::CreateFunctionCallNode(UEdGraph* Graph, UFunction* Function, const FVector2D& Position)
{
    if (!Graph || !Function)
    {
        return nullptr;
...
UK2Node_VariableGet* FPirateMCPCommonUtils::CreateVariableGetNode(UEdGraph* Graph, UBlueprint* Blueprint, const FString& VariableName, const FVector2D& Position)
{
    if (!Graph || !Blueprint)
    {
        return nullptr;
...
UK2Node_VariableSet* FPirateMCPCommonUtils::CreateVariableSetNode(UEdGraph* Graph, UBlueprint* Blueprint, const FString& VariableName, const FVector2D& Position)
{
    if (!Graph || !Blueprint)
    {
        return nullptr;
...
UK2Node_InputAction* FPirateMCPCommonUtils::CreateInputActionNode(UEdGraph* Graph, const FString& ActionName, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
...
UK2Node_Self* FPirateMCPCommonUtils::CreateSelfReferenceNode(UEdGraph* Graph, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
...
bool FPirateMCPCommonUtils::ConnectGraphNodes(UEdGraph* Graph, UEdGraphNode* SourceNode, const FString& SourcePinName, 
...
UEdGraphPin* FPirateMCPCommonUtils::FindPin(UEdGraphNode* Node, const FString& PinName, EEdGraphPinDirection Direction)
{
    if (!Node)
    {
        return nullptr;
    ...
    UE_LOG(LogPirateMCP, Display, TEXT("FindPin: Looking for pin '%s' (Direction: %d) in node '%s'"), 
           *PinName, (int32)Direction, *Node->GetName());
    
    for (UEdGraphPin* Pin : Node->Pins)
    {
        UE_LOG(LogPirateMCP, Display, TEXT("  - Available pin: '%s', Direction: %d, Category: %s"), 
    ...
    for (UEdGraphPin* Pin : Node->Pins)
    {
        ...
        {
            UE_LOG(LogPirateMCP, Display, TEXT("  - Found exact matching pin: '%s'"), *Pin->PinName.ToString());
            return Pin;
        }
    }
    ...
    for (UEdGraphPin* Pin : Node->Pins)
    {
        ...
        {
            UE_LOG(LogPirateMCP, Display, TEXT("  - Found case-insensitive matching pin: '%s'"), *Pin->PinName.ToString());
            return Pin;
        }
    }
    ...
    if (Direction == EGPD_Output && Cast<UK2Node_VariableGet>(Node) != nullptr)
    {
        for (UEdGraphPin* Pin : Node->Pins)
        {
            ...
            {
                UE_LOG(LogPirateMCP, Display, TEXT("  - Found fallback data output pin: '%s'"), *Pin->PinName.ToString());
                return Pin;
            }
        }
    }
    
    UE_LOG(LogPirateMCP, Warning, TEXT("  - No matching pin found for '%s'"), *PinName);
    return nullptr;
}

// Actor utilities
TSharedPtr<FJsonValue> FPirateMCPCommonUtils::ActorToJson(AActor* Actor)
{
    if (!Actor)
    {
...
TSharedPtr<FJsonObject> FPirateMCPCommonUtils::ActorToJsonObject(AActor* Actor, bool bDetailed)
{
    if (!Actor)
    {
        return nullptr;
...
UK2Node_Event* FPirateMCPCommonUtils::FindExistingEventNode(UEdGraph* Graph, const FString& EventName)
{
    if (!Graph)
    {
        return nullptr;
    ...
    for (UEdGraphNode* Node : Graph->Nodes)
    {
        ...
        if (EventNode && EventNode->EventReference.GetMemberName() == FName(*EventName))
        {
            UE_LOG(LogPirateMCP, Display, TEXT("Found existing event node with name: %s"), *EventName);
            return EventNode;
        }
    }
...
bool FPirateMCPCommonUtils::SetObjectProperty(UObject* Object, const FString& PropertyName, 
    ...
    else if (Property->IsA<FByteProperty>())
    {
        ...
        if (EnumDef)
        {
            ...
            if (Value->Type == EJson::Number)
            {
                ...
                UE_LOG(LogPirateMCP, Display, TEXT("Setting enum property %s to numeric value: %d"), 
            ...
            else if (Value->Type == EJson::String)
            {
                ...
                if (EnumValueName.IsNumeric())
                {
                    ...
                    UE_LOG(LogPirateMCP, Display, TEXT("Setting enum property %s to numeric string value: %s -> %d"), 
                ...
                if (EnumValue != INDEX_NONE)
                {
                    ...
                    UE_LOG(LogPirateMCP, Display, TEXT("Setting enum property %s to name value: %s -> %lld"), 
                ...
                else
                {
                    ...
                    UE_LOG(LogPirateMCP, Warning, TEXT("Could not find enum value for '%s'. Available options:"), *EnumValueName);
                    for (int32 i = 0; i < EnumDef->NumEnums(); i++)
                    {
                        UE_LOG(LogPirateMCP, Warning, TEXT("  - %s (value: %d)"), 
    ...
    else if (Property->IsA<FEnumProperty>())
    {
        ...
        if (EnumDef && UnderlyingNumericProp)
        {
            ...
            if (Value->Type == EJson::Number)
            {
                ...
                UE_LOG(LogPirateMCP, Display, TEXT("Setting enum property %s to numeric value: %lld"), 
            ...
            else if (Value->Type == EJson::String)
            {
                ...
                if (EnumValueName.IsNumeric())
                {
                    ...
                    UE_LOG(LogPirateMCP, Display, TEXT("Setting enum property %s to numeric string value: %s -> %lld"), 
                          *PropertyName, *EnumValueName, EnumValue);
                    return true;
                }
